...
LOGLEVEL="info"
# Set dedicated home directory for the dhivesd instance
HOMEDIR="$HOME/.tmp-dhivesd"
# to trace evm
#TRACE="--trace"
TRACE=""
...
while [[ $# -gt 0 ]]; do
	...
	-n)
		...
		;;
	--no-install)
		echo "Flag --no-install passed -> Skipping installation of the dhivesd binary."
		install=false
		shift # Move past the flag
		;;
	*)
...
if [[ $overwrite == "y" || $overwrite == "Y" ]]; then
	...
	# Set client config
	dhivesd config set client chain-id "$CHAINID" --home "$HOMEDIR"
	dhivesd config set client keyring-backend "$KEYRING" --home "$HOMEDIR"
	...
	echo "$VAL_MNEMONIC" | dhivesd keys add "$VAL_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$HOMEDIR"
	echo "$USER1_MNEMONIC" | dhivesd keys add "$USER1_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$HOMEDIR"
	echo "$USER2_MNEMONIC" | dhivesd keys add "$USER2_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$HOMEDIR"
	echo "$USER3_MNEMONIC" | dhivesd keys add "$USER3_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$HOMEDIR"
	echo "$USER4_MNEMONIC" | dhivesd keys add "$USER4_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$HOMEDIR"

	# Set moniker and chain-id for Evmos (Moniker can be anything, chain-id must be an integer)
	dhivesd init $MONIKER -o --chain-id "$CHAINID" --home "$HOMEDIR"
	...
	dhivesd add-genesis-account "$(dhivesd keys show "$VAL_KEY" -a --keyring-backend "$KEYRING" --home "$HOMEDIR")" 100000000000000000000000000$BASE_DENOM --keyring-backend "$KEYRING" --home "$HOMEDIR"
	dhivesd add-genesis-account "$(dhivesd keys show "$USER1_KEY" -a --keyring-backend "$KEYRING" --home "$HOMEDIR")" 1000000000000000000000$BASE_DENOM --keyring-backend "$KEYRING" --home "$HOMEDIR"
	dhivesd add-genesis-account "$(dhivesd keys show "$USER2_KEY" -a --keyring-backend "$KEYRING" --home "$HOMEDIR")" 1000000000000000000000$BASE_DENOM --keyring-backend "$KEYRING" --home "$HOMEDIR"
	dhivesd add-genesis-account "$(dhivesd keys show "$USER3_KEY" -a --keyring-backend "$KEYRING" --home "$HOMEDIR")" 1000000000000000000000$BASE_DENOM --keyring-backend "$KEYRING" --home "$HOMEDIR"
	dhivesd add-genesis-account "$(dhivesd keys show "$USER4_KEY" -a --keyring-backend "$KEYRING" --home "$HOMEDIR")" 1000000000000000000000$BASE_DENOM --keyring-backend "$KEYRING" --home "$HOMEDIR"

	# Sign genesis transaction
	dhivesd gentx "$VAL_KEY" 1000000000000000000000$BASE_DENOM --gas-prices ${BASEFEE}$BASE_DENOM --keyring-backend "$KEYRING" --chain-id "$CHAINID" --home "$HOMEDIR"
	## In case you want to create multiple validators at genesis
	## 1. Back to `dhivesd keys add` step, init more keys
	## 2. Back to `dhivesd add-genesis-account` step, add balance for those
	## 3. Clone this ~/.dhivesd home directory into some others, let's say `~/.clonedEvmosd`
	## 4. Run `gentx` in each of those folders
	## 5. Copy the `gentx-*` folders under `~/.clonedEvmosd/config/gentx/` folders into the original `~/.dhivesd/config/gentx`

	# Collect genesis tx
	dhivesd collect-gentxs --home "$HOMEDIR"

	# Run this to ensure everything worked and that the genesis file is setup correctly
	dhivesd validate-genesis --home "$HOMEDIR"
